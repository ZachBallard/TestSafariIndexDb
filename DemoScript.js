...
var previewOfImageToUpload = function() {
    ...
    var imageUrl = urlCreator.createObjectURL(imageToUpload.Content);
    $("#imageToUpload").attr("src", imageUrl);
}
...
var uploadImage = function (file) {
    if (checkForImageFile(file) && file.files.length > 0) {
		...
		imageToUpload.Content = file.files[0];
    ...
    getImages(connection).done(function (images) {
        ...
        if (images.length == 0) {
            ...
            $.each(images, function (i, image) {
                $("#uploadedImageList .panel-body").append('<div class="image-thumbnail col-sm-2" data-id="' + image.ImageId + '"><img src="' + urlCreator.createObjectURL(imageToUpload.Content) + '" /></div>');
            });
            $("#btnClearImages").show();
        }
    ...
    $(document).on("click", "#uploadedImageList .image-thumbnail", function () {
        getImageById(connection, $(this).attr("data-id")).done(function (image) {
            if (image.length > 0) {
                ...
                $("#selectedImagePreview .modal-body").html('<img src="' + urlCreator.createObjectURL(image[0].ImageContent) + '"/>')
